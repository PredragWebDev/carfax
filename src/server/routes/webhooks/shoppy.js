...
router.post("/", async (req, res) => {
  // TODO: Verify "x-shoppy-signature" header
  const { event } = req.body;

  if (event === "product:fetch") {
    const { order } = req.body.data;
    const { email, product, custom_fields } = order;
    ...
    if (custom_fields && custom_fields.length > 0) {
      for (const { name, value } of custom_fields) {
        if (name === "VIN") {
          ...
          if (error) {
            ...
            log({
              status: "error",
              type: "Shoppy Webhook: Purchased Single Report",
              data: JSON.stringify(
      ...
      if (PLATE && STATE) {
        ...
        if (VINFromPlate.error) {
          ...
          if (error) {
            ...
            //console.log(`Report ID: ${report.id.toUpperCase()}`);
  ...
  } else if (event === "simulator") {
    res.send("Simulator test success!");
  } else {
    res.send("Unknown");
  }
});
